...
class UpdateRestaurantUseCaseTest {
    ...
    @BeforeEach
    void setUp() {
        updateRestaurantUseCase = new UpdateRestaurantUseCase(restaurantRepository, userRepository, authenticatedUserProvider);
    }
    ...
    @Test
    void shouldUpdateRestaurantWhenLoggedUserIsOwner() {
        ...
        Restaurant result = updateRestaurantUseCase.execute(
                restaurantId, "Pizzaria Napoli", "Rua Nova, 789",
                LocalTime.of(17, 0), LocalTime.of(22, 0));

        assertEquals("Pizzaria Napoli", result.getName());
        assertEquals("Rua Nova, 789", result.getAddress());
    }
